...
# ML — ⚠️ PyCaret full은 scikit-learn/numpy 버전 고정으로 Streamlit과 충돌함
# slim 설치 후 필요 모델만 개별 설치하는 것이 안전
pycaret = "^3.3"
# PyCaret slim이 포함하지 않는 모델은 개별 설치
xgboost = "^2.0"
lightgbm = "^4.3"
optuna = "^3.5"  # 하이퍼파라미터 튜닝

# DL (선택적)
autokeras = {version = "^2.0", optional = true}
...
class AnalysisResult:
    ...
    insights: list[str]                         # AI 생성 인사이트 3줄
...
class ReviewAnalyzer:
    ...
    Usage:
        ...
        analyzer.save_delivery_package(result)
    """
    ...
    def save_delivery_package(self, crawl_result: CrawlResult, analysis_result: AnalysisResult, output_dir: Path) -> Path:
        """크몽 납품 패키지 생성.
...
analysis:
  ...
  sentiment_method: rating_based  # rating_based | keyword_based | hybrid
...
# Streamlit에서 진행률 표시:
# st.spinner + threading 조합 사용
import threading

def crawl_with_streamlit_progress(config, progress_bar, status_text):
    '''Streamlit UI에서 진행률 표시하며 크롤링.'''
    result_container = {}
    ...
    def _run():
        ...
        engine = CrawlerEngine(config)
        async def _crawl():
            async for update in engine.run_with_progress():
                result_container['progress'] = update
            result_container['result'] = await engine.run()
    ...
    while thread.is_alive():
        if 'progress' in result_container:
            p = result_container['progress']
            progress_bar.progress(p.get('progress', 0))
            status_text.text(f"수집 중: {p.get('collected', 0)}건")
        time.sleep(0.5)
    thread.join()
...
class AutoMLRunner:
    ...
    Usage:
        ...
        result = runner.run()
        runner.save_delivery_package(result)
    """
    ...
    def run_with_progress(self) -> Any:
        """Streamlit UI용 진행률 포함 실행.

        Yields:
            {"step": "comparing_models", "progress": 0.3, "current_model": "RandomForest"}
        """
        ...

    def save_delivery_package(self, result: AutoMLResult) -> Path:
        """납품 패키지 생성.
...
2. **GCP Cloud Run** — 대규모 분석 (네 GCP 경험 활용) → $0.00002/초
...
진영이 GCP 경험이 있으니 이게 가장 현실적인 유료 배포 옵션.
